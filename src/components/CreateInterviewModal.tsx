  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error ?? '作成に失敗しました。入力内容をご確認ください。')
        return
      }
      track('interview_created', { type: sessionType })
      onCreated()
    } catch {
    ...
    >
      ...
      >
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h2 id="create-interview-title" className="text-base font-semibold tracking-tight text-gray-900">インタビューを手動で作成</h2>
