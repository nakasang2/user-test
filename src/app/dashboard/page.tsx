...
export default function Dashboard() {
  ...
  // インタビューごとにセッションをグループ化
  const grouped = interviews.map((iv) => ({
    ...iv,
    sessions: sessions.filter((s) => s.interview.id === iv.id),
  }))
                ...
                return (
                  <div key={iv.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                    ...
                    <div className="px-4 py-3 flex items-center justify-between gap-2">
                      ...
                      >
                        <span className={`text-gray-500 transition-transform text-xs ${isCollapsed ? '-rotate-90' : ''}`}>
                          ▼
                        </span>
                          ...
                          <div className="divide-y divide-gray-800/60">
                            {iv.sessions.map((s) => (
                              <div key={s.id} className="relative group">
                                ...
                                >
                                  {/* 被験者名 */}
                                  <span className="text-sm font-medium text-gray-200 min-w-0 flex-1 truncate">
                                    {s.participant?.name ?? 'Anonymous'}
                                  </span>

                                  {/* サマリー（あれば） */}
                                  {s.transcript?.summary && (
                                    <span className="hidden md:block text-xs text-gray-500 flex-1 truncate">
                                      {s.transcript.summary}
                                    </span>
                                  )}
