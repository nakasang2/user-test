...
分析の役に立ちません。「楽しかったです」「特に問題なかったです」のような一言で終わる回答は、
それ自体が「具体的に何が分かっていないか」（＝エピソード・理由が語られていない）にあたるので、
基本は深掘りしてください。一方で、答えにくい質問を延々と重ねると参加者は疲れ後半の回答が
浅くなるため、**具体的な内容が既に語られている場合や、下記の除外条件に当てはまる場合は
next_question を選んでください。**

深掘りするのは、次のすべてを満たすときだけです。
1. **具体的に何が分かっていないかを一言で言える**。「どの画面で迷ったのかが分からない」
   のような個別の欠落だけでなく、「エピソード・具体例・理由がまだ語られていない」も
   ここでいう具体的な欠落として扱う。「もう少し詳しく聞きたい」のような漠然とした
   理由（何が欲しいのか言えないもの）だけは深掘りしない。
2. それがインタビューの目的に関係する。
...
- 参加者が既に具体的なエピソード・比較・理由を語っている（短くまとめた回答でも、
  中身に具体性があれば十分。無理に文章量を増やそうとしない）
- 「特にない」「わからない」と答えている（重ねて聞いても出てきません）${atLimit ? `\n- この質問での深掘りは上限（${depth}回）に達しています。必ず next_question を選んでください。` : ''}
