...
## 進行中
- ユーザビリティ操作を小窓に一本化。第1弾 c806969 は PR #8 で main マージ済み（本番反映済み）。第2弾以降は [PR #9](https://github.com/nakasang2/user-test/pull/9) でマージ待ち: 第2弾 b8360c4（段階表示・番号廃止・終了ボタン常時表示）、第3弾 8a03e65（タスク文言を事前手続き後に表示／メイン3ステップ進捗）、第4弾（顔フレーミング判定＝小窓で見切れ/未検出をアラート）を実装・ローカル検証済みで push 予定。
